...
namespace dev
{
	namespace solidity
	{
		...
		void ExpressionCompiler::appendStateVariableAccessor(VariableDeclaration const &_varDecl)
		{
			...
			if (retSizeOnStack > 15)
				BOOST_THROW_EXCEPTION(
					CompilerError() << errinfo_sourceLocation(_varDecl.location()) << errinfo_comment("Stack too deep."));
			...
			else // compound assignment
			{
				...
				if (lvalueSize > 0)
				{
					if (itemSize + lvalueSize > 16)
						BOOST_THROW_EXCEPTION(
							CompilerError() << errinfo_sourceLocation(_assignment.location()) << errinfo_comment("Stack too deep, try removing local variables."));
			...
			else
			{
				...
				case FunctionType::Kind::ArrayPush:
				{
					...
					shared_ptr<ArrayType> arrayType =
						function.kind() == FunctionType::Kind::ArrayPush ? make_shared<ArrayType>(DataLocation::Storage, paramType) : make_shared<ArrayType>(DataLocation::Storage);
		...
		void ExpressionCompiler::appendCompareOperatorCode(Token::Value _operator, Type const &_type)
		{
			...
			else
			{
				...
				case Token::GreaterThanOrEqual:
					m_context << (isSigned ? Instruction::SLT : Instruction::LT) << Instruction::ISZERO;
					break;
				case Token::LessThanOrEqual:
					m_context << (isSigned ? Instruction::SGT : Instruction::GT) << Instruction::ISZERO;
					break;
			...
			vector<ASTPointer<Expression const>> const &_arguments)
		{
			...
			bool returnSuccessCondition = funKind == FunctionType::Kind::BareCall || funKind == FunctionType::Kind::BareCallWithPay || funKind == FunctionType::Kind::BareCallCode || funKind == FunctionType::Kind::BareDelegateCall || funKind == FunctionType::Kind::TransferEx || funKind == FunctionType::Kind::SendEx;
